...
### 跨命名空间引用
当一个问题涉及多个层面时（如"调整定价是否影响定位"），按以下顺序检索：
1. 查 **00_Strategy** → 确认定位和原则是否允许
2. 查 **01_Knowledge** → 查产品分析和财务模型
3. 查 **02_Operations** → 查当前运营数据
4. 综合后输出结论

### 更新规则
- 运营方案新增/修改文件 → 同步更新本索引
- 索引本身属于 00_Strategy（永久保留），不可自动清理
- 版本号与 `农场运营方案/` 的版本号保持一致
